...
import 'package:simple_pos/features/sales/domain/entities/chart_enums.dart';
...
class TrendChartWidget extends StatelessWidget {
  ...
  const TrendChartWidget({
    ...
    this.selectedMetric = ChartMetric.revenue,
    this.onMetricChanged,
  });

  double _getValueByMetric(SalesTrendData data) {
    switch (selectedMetric) {
      ...
      case ChartMetric.transactions:
        return data.transactions.toDouble();
    }
  }
  ...
  double _getMaxY(SalesTrendAnalysis analysis) {
    ...
    for (final data in analysis.historicalData) {
      final value = _getValueByMetric(data);
  ...
  String _formatCurrency(double value) {
    if (selectedMetric == ChartMetric.transactions) {
        ...
        children: ChartMetric.values.map((metric) {
          ...
          return Expanded(
            child: GestureDetector(
              onTap: () => onMetricChanged?.call(metric),
              child: Container(
